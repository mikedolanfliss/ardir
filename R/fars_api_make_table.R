...
# fars_request_tbl |> saveRDS("data/fars_request_tbl.rds") # 86 megs, 1.3M rows x 162 variables
...
fars_person_sm_tbl = fars_person_sm_tbl |> 
  ...
  mutate(direct_alc_attributable = case_when( # ID >0.08 BAC CRASHES ####
      alc_bac_result > 8 ~ TRUE, # "Directly Alcohol Attributable"
      T ~ FALSE)) # "Other / Mixed Cause"
# TODO question - drop missing age -> missing age group?

fars_person_sm_tbl |> count(alc_bac_result, alc_bac_text) |> arrange(desc(alc_bac_result)) # Could save and quarto a table like this. 
